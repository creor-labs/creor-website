...
import { DocsBreadcrumbNav } from "@/components/docs/docs-breadcrumb-nav";
...
export function DocsPage({ breadcrumb, title, description, toc, children }: DocsPageProps) {
  return (
    <>
      <div className="mx-auto max-w-[720px] px-5 py-10 sm:px-6 lg:py-16">
        <DocsBreadcrumbNav breadcrumb={breadcrumb} />

        <h1 className="mb-5 text-[28px] font-semibold tracking-tight text-[#EDEDED] sm:text-[36px]">
          {title}
        </h1>
        <p className="mb-10 text-[15px] leading-relaxed text-[#A1A1A1] sm:text-[16px]">
          {description}
        </p>
      ...
      <DocsFloatingToc toc={toc ?? []} breadcrumb={breadcrumb} />
      <DocsNavPill breadcrumb={breadcrumb} />
    </>
  );
}
